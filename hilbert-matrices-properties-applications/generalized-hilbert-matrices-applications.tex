...
\begin{align*} 2(g^{oo} g^{ee}-(g^{oe})^2) &= 2(N(p+N)(g^{oo}) - d^2)\\ &= \sum_{i,j \text{ odd}}^N \left( \frac{2*N(p+N)}{p+i+j-1} - 1\right) \frac{\prod_{k=0}^{N-1} (p+i+k)(p+j+k)}{(i-1)! (N-i)! (j-1)! (N-j)!} \\
...
&= \sum_{i,j \text{ odd}}^N (N-1) \dbinom{N-1}{i-1} \dbinom{N-j}{j-1} \end{align*}
 which clearly grows at least as large as $O(N^3)$. Since our numerator grows like $O(N^2)$, we are done.

% thus begins a LONNG COMMENT

\begin{comment} Note that $N-i + i-1 = N-1$, so at least one of them must be less than $N/2$. Thus $\frac{(N-1)!}{(N-i)!(i-1)!} \ge 2^{\min(N-i,i-1)}$. Now, for any positive $i,p$, 
\begin{align*}
\prod_{k=0}^{N-1} p+i+k & \ge (N-1)! \\
& >= (i-1)!(N-i)! 
\end{align*} and thus our lower bound from our last inequality is greater than 
\begin{align*}
\sum_{i,j \text{ odd}}^N N-1 = \frac{N^3-N^2}{4}
\end{align*} which grows like $N^3$

Now, consider $A(i,j) = \frac{\prod_{k=0}^{N-1} (p+i+k)(p+j+k)}{(i-1)! (N-i)! (j-1)! (N-j)!}$. For $i<N$, 
\begin{align*}
A(i+1,j) &= \frac{\prod_{k=0}^{N-1} (p+i+k+1)(p+j+k)}{(i)! (N-i-1)! (j-1)! (N-j)!}\\
&= \frac{(N-i)(p+N+i)}{i(p+i)}\frac{\prod_{k=0}^{N-1} (p+i+k)(p+j+k)}{(i-1)! (N-i)! (j-1)! (N-j)!}\\
&=\frac{(N-i)(p+N+i)}{i(p+i)} A(i,j)
\end{align*}\end{comment}
% thus ends a LONNG COMMENT
