...
\end{align}
decays to 0 as $N\to \infty$. Select some $N > 4k$, and let $K = \{i \; \text{odd}: k  \le i \le N/2\}$. We claim that for any $i\in K$, 
...
\end{proof}
Since there are at least $N/8-1$ such values in $K$ when $N>4k$, our denominator in \ref{coefficient-expanded-ratio} is at least $N/16$ times the numerator, and therefore our ratio in \ref{coefficient-expanded-ratio} is bounded by $\frac{16}{N}$. Now to prove the inequality \ref{inequality-sum-of-products}. We will show that our the summands of our expression, considered as a function of $i$ (with fixed $j$, increases on the interval $[1, N/2]$. In fact, assuming the sign is positive, we write 
\begin{align*}
&(N(p+N)-(p+i+j-1)) G_{ij}\\
&= (N(p+N)-(p+i+j-1))(p+i+j-1)\dbinom{N+p+i-1}{N-j} \dbinom{N+p+j-1}{N-i} \dbinom{p+i+j-2}{i-1} \dbinom{p+i+j-2}{j-1}
\end{align*} We will show that this expression as a function of $i$ is increasing on the interval $[1,N/2]$. First assume that $j>1$. First note that $\dbinom{N+p+i-1}{N-j}$ is clearly increasing in $i$. Now, we can write \begin{align*}
(p+i+j-1)\dbinom{N+p+j-1}{N-i}\dbinom{p+i+j-2}{i-1} &= (p+i+j-1)\frac{(N+p+j-1)!(p+i+j-2)!}{(N-i)!(p+i+j-1)!(i-1)!(p+j-1)!}\\
&= \frac{(N+p+j-1)!}{(N-i)!(i-1)!(p+j-1)!}\\
&= \frac{(N+p+j-1)!}{(p+j-1)!}\frac{1}{(N-1)!}\dbinom{N-1}{i-1}
\end{align*} which depends only on $i$ in the binomial term, and thus is increasing in $i$ whenever $i-1 < (N-1)/2$, guaranteed on the interval $[1,N/2]$. It remains to show that 
\[f(i) = (N(p+N)-(p+i+j-1))\dbinom{p+i+j-2}{j-1}\]
increases in $i$. But 
\begin{align*}
\frac{f(i+1)}{f(i)} &= \frac{(N(p+N)-(p+i+j))\dbinom{p+i+j-1}{j-1}}{(N(p+N)-(p+i+j-2))\dbinom{p+i+j-2}{j-1}}\\
&= \frac{(N(p+N)-(p+i+j))\frac{(p+i+j-1)!}{(j-1)!(p+i)!}}{(N(p+N)-(p+i+j-1))\frac{(p+i+j-2)!}{(j-1)!(p+i-1)!}}\\
&= \frac{(N(p+N)-(p+i+j))(p+i+j-1)}{(N(p+N)-(p+i+j-1))(p+i)}
\end{align*}
Note that $p+i+j-1 > p+i-1$ when $j>1$. Furthermore, for $N>4$, the terms $N(p+N)-(p+i+j) > p+i+j-1$. Thus this fraction can be rewritten as 
\[\frac{A+1}{A} \frac{B+c}{B}\]
where $A>B$ and $c\ge 1$. Clearly, then, $\frac{B+c}{B} > \frac{A+1}{A}$ and so $\frac{f(i+1)}{f(i)} > 1$ and is therefore increasing in $i$. 
Thus, for $j>1$, we have given groupings of the factors that are all increasing in $i$ on the interval $[1,N/2]$ (for loose conditions on the lower bound of $N$). \\
To finish we simply consider the case when $j=1$. Then we can simplify our expression as
\begin{align*}
(N(p+N)-(p+i))G_{i1} &= (N(p+N)-(p+i))(p+i)\dbinom{N+p+i-1}{N-1}\dbinom{N+p}{N-i}\dbinom{p+i-1}{i-1}
\end{align*}
